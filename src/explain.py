...
def create_shap_waterfall_chart(df_shap: pd.DataFrame, max_features: int = 6) -> go.Figure:
    ...
    colors = ["#EF4444" if s > 0 else "#0284C7" for s in df_top["shap_value"]]
    ...
    fig.update_layout(
        title=dict(text="<b>SHAP Feature Contribution Analysis</b>", font=dict(size=15, color="#F8FAFC")),
        xaxis=dict(title="SHAP Value (Risk Contribution)", gridcolor="rgba(255,255,255,0.08)", zerolinecolor="rgba(255,255,255,0.2)"),
